...
describe('Testing class routes', () => {
  test('test add announcement should return unauthenticated', (done) => {
    supertest(app)
      .post('/api/v1/class/1/announcement')
      .set('Cookie', [
  ...
  test('test add announcement should return description', (done) => {
    supertest(app)
      .post('/api/v1/class/1/announcement')
      .send({
      ...
      .end((err, res) => {
        if (err) return done(err);
        expect(res.body.data.description).toEqual('test test test');
        return done();
      });
  });

  test('test add announcement should return description is required', (done) => {
    supertest(app)
      .post('/api/v1/class/1/announcement')
      .send({
        description: '',
      ...
      .end((err, res) => {
        if (err) return done(err);
        expect(res.body.msg).toEqual('description is required');
        return done();
      });
  });

  test('test add announcement should return Internal server error!', (done) => {
    supertest(app)
      .post('/api/v1/class/asd/announcement')
      .send({
        description: 'test test test',
      })
      .set('Cookie', [

...
import { WhatsAppOutlined, MailOutlined, AppstoreAddOutlined} from '@ant-design/icons'
import './style.css'
  ...
  const ProfileCard:ElementType = ({data, title, type}:ProfileCardCC) => {
    ...
    return (
      <div className='profile_card'>
         ...
         <h1>{title}</h1>
         <p>{subtitle1}<span className='teachers_number'>{' '} {data.length}{' '} </span> {subtitle2}</p>
         </div>
          <div
         id="scrollableDiv"
         style={{
           ...
           <List
             ...
             renderItem={item => (
               <List.Item key={item.mobile}>
                 ...
                 <div className='icons'>
                  {
                    type === 'students' ?<><MailOutlined /> <WhatsAppOutlined /></> : <AppstoreAddOutlined />
                  }
                 </div>
               </List.Item>
             )}

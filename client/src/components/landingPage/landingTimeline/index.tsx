...
const TimeLineSection: FC = () => {
  ...
  <section id="timeline-section">
      <div className="timeline-text">
        <h1>{role}</h1>
      {data.map((e: any) => <Timeline.Item>{e}</Timeline.Item>)}
      <Timeline.Item>
    ...
    <div className="timeline-buttons">
      <div className="buttons">
        <div><button type="button" name="student" onClick={handleData}>طالب</button></div>
        <div><button type="button" name="teacher" onClick={handleData}>مدرس</button></div>
        <div><button type="button" name="parent" onClick={handleData}>ولي أمر</button></div>
      </div>
    </div>
  </section>
);}
